...
function rs_add_meta_boxes() {
    add_meta_box( 'case-study-bg', esc_html__('Slider Settings', 'rsframework'), 'rs_header_bg', 'page', 'normal', 'high' );    
...
function add_sidebar_metabox()
{
    add_meta_box( 
        'custom_sidebar',
        esc_html__( 'Post Sidebar Setting', 'rsframework' ),
...
function custom_sidebar_callback( $post ){
  ...
  $output1   = '<div class="radio-select"><p><label for="myplugin_layout">'.esc_html__("Choose Layout", 'rsframework' ).'</label></p>';
  ...
  $output = '<p><label for="myplugin_new_field">'.esc_html__("Choose a sidebar to display", 'rsframework' ).'</label></p>';    
  $output .= "<select name='custom_sidebar'>";
    ...
    if($val == "sidebar-1")
      ...
      $output .= " value='sidebar-1'>".esc_html__('Select Sidebar','rsframework')."</option>";
